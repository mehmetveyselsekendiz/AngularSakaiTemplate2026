...
```bash
npm test               # zoneless birim testleri (Karma + Jasmine, headless Chrome)
npm run test:watch     # izleme modu (geliştirme)
npm run test:scripts   # governance script'leri (saf Node, node:test)
```

Sıfır yeni paket — zaten kurulu Karma+Jasmine. Birim testleri `core/` davranışını
(settings/i18n/auth) kapsar; her spec `provideZonelessChangeDetection()` sağlar ve
effect'leri `ApplicationRef.tick()` ile flush eder (zone.js yok). Governance
script'leri (`check-palette`/`check-tokens-sync`) `MFA_LINT_ROOT` env override'ı ile
sahte fixture köklerine yönlendirilerek drift/ihlal → exit 1 negatif testleriyle doğrulanır.

**CI:** `.github/workflows/ci.yml` her push/PR'da `lint:palette` → `lint:tokens` → snippet tazeliği → `test:scripts` → `test` → `build` kapısını çalıştırır (Node 22). Bu kapı yeşil olmadan merge edilmez.
