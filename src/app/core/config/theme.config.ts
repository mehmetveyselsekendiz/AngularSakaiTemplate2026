...
export const MfaPreset = definePreset(Aura, {
    ...
    semantic: {
        ...
        colorScheme: {
            ...
            dark: {
                // Aura/Sakai konvansiyonu: surface scale tema-BAĞIMSIZ.
                // Sayfa pattern'i `bg-surface-50 dark:bg-surface-950`:
                //   light → `--p-surface-50` (açık) = #f9f9fa zemin
                //   dark  → `--p-surface-950` (koyu) = #0d0e10 zemin
                // Yani aynı index, iki temada farklı işlev (sayfa hangi index'i kullandığına göre).
                // Aura kendi `content`/`text` token'larını tema-bağımlı yönetir; biz sadece palet veriyoruz.
                surface: {
                    0: '#ffffff',
                    50: '#f9f9fa',
                    100: '#f2f2f3',
