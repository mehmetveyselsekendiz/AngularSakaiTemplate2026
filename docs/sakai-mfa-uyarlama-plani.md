...
### Phase 9 (Yeniden) — Bileşen Vitrini + Kütüphane Zenginleştirme (SIRADA — aktif)
...
- [ ] **9A** — `ComponentShowcase` (kart + header kopyala + `p-tabs` Önizleme/Kod, MFA kimliği, dark-aware) + buttondemo/inputdemo pilot
- [ ] **9B** — Kalan 15 uikit sayfasında **her** örnek showcase'e (~105 toplam)
- [ ] **9C** — Gap analizi + eksik PrimeNG bileşen demoları (form/tablo/overlay/feedback öncelikli)
- [ ] **9D** — "Kurumsal Desenler" sayfası (8 kompozit kalıp: page header, tablo+toolbar, form kartı, empty state, stat cards, filtre çubuğu, onay akışı, detay paneli)
...
Yeni Claude Code oturumunda **`docs/yeni-sakai-session-prompt.md`** dosyasını oku ve ilk mesaj olarak yapıştır. **Tamamlanan:** Phase 0–8 + 9.0 (kısmi). **Aktif faz:** Phase 9 (Yeniden) — Bileşen Vitrini + Kütüphane Zenginleştirme (tasarım: `superpowers/specs/2026-05-25-phase-9-component-library-design.md`, plan: `superpowers/plans/2026-05-25-phase-9-component-library.md`). Faz ilerlemesi için [`ilerleme-ve-kararlar.md`](ilerleme-ve-kararlar.md).
