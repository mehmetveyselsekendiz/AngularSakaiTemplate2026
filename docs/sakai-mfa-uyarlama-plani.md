...
### Phase 9 — Bileşen Kodunu Görüntüleme/Kopyalama (TAMAMLANDI — 24-25 Mayıs 2026)
**Karar:** build-script extraction (K-016) — manuel string / `?raw` yerine.

- [x] `scripts/extract-snippets.mjs` (sıfır-paket) + `npm run snippets` — `<!-- snippet:ID -->` bloklarını `public/snippets/<sayfa>.json`'a çıkarır
- [x] `CodeBlock` component (`uikit/code-block.ts`) — "Kodu Göster/Gizle" + "Kopyala", dark-aware, syntax highlight yok
- [x] `SnippetService` (`uikit/snippet.service.ts`) — JSON yükler (HttpClient + signal)
- [x] `/uikit/*` 17 sayfanın hepsinde ana örnek işaretlendi (18 snippet); tarayıcıda doğrulandı
- [ ] (Opsiyonel, sonraki) `npm run snippets`'i prebuild'e bağla; sayfa başına birden fazla örnek
...
Yeni Claude Code oturumunda **`docs/yeni-sakai-session-prompt.md`** dosyasını oku ve ilk mesaj olarak yapıştır. **Tamamlanan:** Phase 0–9. **Aktif faz:** Phase 10 — Responsive Audit. Faz ilerlemesi için [`ilerleme-ve-kararlar.md`](ilerleme-ve-kararlar.md).
