...
## Oturum 11 — 30 Mayıs 2026

### Kurumsal Kimlik Bütünlüğü Denetimi + Eksik Tamamlama (Tamamlandı)

**Bağlam:** Kullanıcı template'i amaç açısından denetledi: "kurumsal kimlik tam kullanılmış mı, kütüphane yeterli mi, yeni modül geliştirmeye uygun mu?" Kod-tabanlı denetimde iki gerçek kurumsal kimlik boşluğu bulundu (font + logo) ve kalan opsiyonel işler tamamlandı.

- [x] **Denetim bulguları:**
  - **Gövde fontu `'Lato'` idi** (`_core.scss:8`) — Helvetica DEĞİL. Phase 1'de Lato CDN'i kaldırılmış ama SCSS kuralı hiç güncellenmemiş; gövde tarayıcı varsayılanına düşüyordu (CLAUDE.md §5 ihlali). `design-tokens.ts`'teki Helvetica stack hiçbir yere bağlı değildi.
  - **Gerçek logo/amblem yoktu** — topbar sadece `<span>MFA</span>` metni; Kurumsal Kimlik sayfası logo varyantlarını yalnızca metin olarak listeliyordu.
  - Governance temiz (lint:palette 0 ihlal), kütüphane kapsamı güçlü (18 sayfa, 125 snippet, IMPORT kuralı makine-zorlamalı) — bunlar amaca uygun bulundu.
- [x] **Font tek-kaynak bağı (K-022):** `mfa-tokens.scss`'e `--mfa-font-sans` / `--mfa-font-serif` token'ları eklendi (design-tokens.ts ile senkron). `_core.scss` body → `font-family: var(--mfa-font-sans)`; ölü `html { font-size: 14px }` kaldırıldı (mfa-tokens `:root` 15px + `data-font-scale` zaten yönetiyor). Denetim sayfası (`/pages/kurumsal-kimlik/denetim`) artık Helvetica'yı yeşil raporlar.
- [x] **Palet-tabanlı SVG amblem (K-023):** `src/app/core/util/mfa-logo.ts` — `MfaLogo` standalone component (`app-mfa-logo`). Inline SVG; renkler `--mfa-brand`/`--mfa-brand-fg` okur (dark-aware, hardcoded hex yok). `variant='full'|'mark'`, `responsive` input. Boyut `--mfa-logo-size` token'ı. Topbar `<span>MFA</span>` → `<app-mfa-logo [responsive]="true" />` (`_topbar.scss` `svg{width:3rem}` → `--mfa-logo-size:2rem`). Kurumsal Kimlik "Logo Varyantları" kartına amblem önizleme (açık/marka zemin) + her varyant `minWebHeightPx` boyutunda render.
- [x] **timelinedemo kırık görsel:** `/demo/images/product/game-controller.jpg` → `svgPlaceholder(200,120,undefined,'Sipariş')`; template `[src]` doğrudan `event.image` okur. Snippet yeniden üretildi (125 snippet, değişmedi).
- [x] **CLAUDE.md K-021 notu:** §8'e dev-data interceptor istisnası açıklaması + §13 yasak listesine istisna referansı eklendi.
- [x] **CI pipeline:** `.github/workflows/ci.yml` — push/PR `main`; Node 22; `npm ci` → `lint:palette` → snippet tazeliği (`git diff --exit-code public/snippets`) → `npm run build`.
- [x] **Doğrulama:** `lint:palette` 0 ihlal (69 dosya), `npm run snippets` 125 snippet/18 sayfa, `npm run build` BAŞARILI (13.05 sn, sadece mevcut Quill CommonJS uyarısı). Sıfır yeni paket.

### Alınan Kararlar

#### K-022 — Font Tek-Kaynak Bağı (30 Mayıs 2026)

**Karar:** Kurumsal font stack'i `mfa-tokens.scss`'te `--mfa-font-sans` token'ı olarak tanımlanır; `body` bunu okur. `design-tokens.ts brandTypography.sans.stack` ile aynı değer (görsel rehber ↔ runtime senkron).

**Gerekçe:** `_core.scss`'teki `'Lato'` kuralı CDN kaldırıldıktan sonra ölü kalmış, gövde varsayılan sans-serif'e düşüyordu — kurumsal kimliğin en temel öğesi (font) hiç uygulanmıyordu. Tek CSS token, renk paletindeki tek-kaynak felsefesini fonta taşır; component'ler hardcoded `font-family` yazmaz.

**Etki:** `body` Helvetica system stack ile render eder. Font değişikliği tek noktadan (`mfa-tokens.scss`). Denetim sayfasının font kontrolü artık yeşil.

#### K-023 — Logo: Palet-Tabanlı SVG Placeholder Bileşeni (30 Mayıs 2026)

**Karar:** Gerçek Bakanlık amblemi gelene kadar `MfaLogo` component'i (`core/util/mfa-logo.ts`) inline SVG yer tutucu sağlar; renkleri MFA paletinden okur. Topbar ve Kurumsal Kimlik sayfası bunu kullanır.

**Gerekçe:** Kurumsal template'in marka öğesi (logo) tamamen eksikti (sadece "MFA" metni). Palet-tabanlı SVG: (1) dark-aware ve governance-temiz (hardcoded hex/CDN yok), (2) tek dosyada gerçek asset ile değiştirilebilir, (3) sıfır-paket. Amblem placeholder bir yıldız-roundel; gerçek amblem geldiğinde `<svg>` içeriği değişir, kullanan yerler değişmez.

**Etki:** Topbar markası + Kurumsal Kimlik logo bölümü canlı amblem gösterir. Yazı markası ("T.C. Dışişleri Bakanlığı" / EN alt başlık) brand wordmark olarak sabit (çevrilmez — K-012 ile uyumlu).

---

## Oturum 12 — 30 Mayıs 2026

### Kütüphane Zenginleştirme — Kurumsal Desenler (Tamamlandı)

**Bağlam:** Kullanıcı, kütüphanenin kurumsal kimliği daha çeşitli ve detaylı kullandığını göstermek istedi; modül takımları dışarıdan paket almadan her ihtiyacı buradan karşılayabilsin. Kod-tabanlı analizde mevcut 8 kompozit kalıbın **iskelet** (sayfa/form/tablo) odaklı olduğu, kurumsal **renk varyasyonu / durum dili / marka aksanı**nın zayıf temsil edildiği tespit edildi.

- [x] **Analiz + onay:** 6 eksik kompozisyon önerildi; kullanıcı "Hepsi (1-6)" onayı verdi.
- [x] **`patternsdemo.ts`'e 6 yeni `<app-showcase>` kalıbı eklendi** (8 → 14):
  - **Durum Rozeti Kataloğu** — iş durumlarının tek-tip `severity`+ikon eşlemesi (success/warn/info/danger/secondary/contrast). Modüller durum renklerini tutarlı tutmak için kopyalar. (`p-tag`)
  - **Marka Aksanlı Bilgi Kartları** — sol kenar marka-renkli (lacivert/kırmızı/altın/gri) şeritli içerik kartları; zemin `color-mix(... transparent)` ile dark-aware. Renk varyasyonunu gösterir. (alias token + `var(--mfa-*)`)
  - **Süreç Adımları (Stepper)** — çok aşamalı başvuru süreci yatay göstergesi. (`p-stepper` — menudemo'da gösterilmiş)
  - **Atama / Kişi Şeridi** — avatar + ad + iletişim + rol rozeti; avatar zemini MFA paletinden. (`p-avatar` + `p-tag`)
  - **Bölümlenmiş Form (Fieldset)** — Kişisel/İletişim/Başvuru gruplarına ayrılmış çok bölümlü Reactive Form. (`p-fieldset`)
  - **İlerleme / Kota Paneli** — tamamlanma %/kontenjan göstergesi; marka renkli `p-progressbar` + durum rozeti. (`p-progressbar`)
- [x] **§14 uyumu:** Tüm yeni kalıplar yalnızca `/uikit`'te zaten gösterilmiş bileşenleri kullanır (stepper=menudemo, avatar/badge/progressbar/tag=miscdemo, fieldset=panelsdemo). Yeni import'lar: `StepperModule`, `AvatarModule`, `BadgeModule`, `ProgressBarModule`.
- [x] **Doğrulama:** `npm run snippets` → **131 snippet / 18 sayfa** (patternsdemo 8→14). `lint:palette` **0 ihlal** (69 dosya). `npm run build` **BAŞARILI** (16.2 sn, sadece mevcut Quill CommonJS uyarısı). Sıfır yeni paket.
- [x] **Tarayıcı doğrulaması:** Geliştirici Modu ile `/uikit/patterns` açıldı — 14 showcase render, 0 konsol hatası; stepper (4 adım), 3 avatar, 3 progressbar, yeni fieldset legend'leri ve durum rozetleri canlı doğrulandı.
- [x] **Tema uyum denetimi (light + dark):** Yeni kalıpların hesaplanmış renkleri token kaynaklarına birebir çözüldü — notice kenarları `#003773`/`#DA291C`/`#D7AD4D`/`#53565A`; avatar zeminleri red/navy/gri; progressbar themed primary (hardcoded değil). Dark mode'da alias token'lar otomatik kaydı (`--mfa-bg #fff→#0d0e10`, `--mfa-text→#f9f9fa`, `--mfa-brand #DA291C→#ef3a34`), showcase kartı `bg-elevated`'a geçti, muted metin okunur kaldı. **Sıfır governance ihlali.**

### Docker / OpenShift Doğrulaması (Tamamlandı)

- [x] **`docker build -t mfa-frontend .` BAŞARILI** (multi-stage: node:22-alpine build → nginx-unprivileged runtime, uid 101). İmaj **84.5 MB**.
- [x] **Runtime testi:** Container `-e SSO_URL=... -e API_URL=... -e CLIENT_ID=...` ile çalıştırıldı — `/health` **HTTP 200 'ok'**; `/config.js` env'den doğru üretildi (`window.__ENV__` SSO/API/CLIENT dolu, default `PORTAL_URL`); `/` ve `/uikit/patterns` (SPA fallback) **200**. `docker-entrypoint.sh` env enjeksiyonu doğrulandı. Önceki oturumda "henüz çalıştırılmadı" sayılan tek açık iş kapatıldı.

**Üretim hazırlığı hükmü:** Tema tek-kaynaktan (renk + font + amblem) yönetiliyor; runtime ayarları tek servisten; governance makine-zorlamalı (lint:palette + IMPORT kuralı + CI); kütüphane 14 kurumsal desen + 18 sayfa/131 snippet; Docker imajı OpenShift-uyumlu ve runtime-config doğrulandı. **Template kurumsal modül geliştirme için production-ready.**

### Token Senkron Doğrulayıcı (Tamamlandı)

- [x] **`scripts/check-tokens-sync.mjs` (K-024):** Marka hex'i teknik olarak 3 dosyada yaşıyor (scss kanonik + theme.config preset + design-tokens.ts) — bunlar elle senkron ve şimdiye dek hiçbir şey ayrışmayı (drift) denetlemiyordu (lint:palette bu 3'ü whitelist'liyor). Sıfır-bağımlılık Node script'i: scss'i kanonik alır, diğer ikisinde 6 marka anchor'ının (red/gold/gray/navy/navy-dark/danger) aynı hex olduğunu doğrular; ayrışmada exit 1. `npm run lint:tokens` + CI adımı (lint:palette'ten sonra).
- [x] **Karar gerekçesi:** Tam codegen ("tek literal" üretici) over-engineering bulundu — Pantone-kilitli palet neredeyse hiç değişmez; kalıcı build karmaşası + insan-okur dosyaların "generated" olması felsefeye ters. Drift-kontrol script'i tek gerçek riski (sessiz ayrışma) %10 maliyetle kapatır, mevcut check-palette.mjs desenine birebir oturur.
- [x] **Doğrulama:** `lint:tokens` ✓ (6 anchor uyumlu: red=#da291c · gold=#d7ad4d · gray=#53565a · navy=#003773 · navyDark=#00235a · danger=#c81d1d).

#### K-024 — Token Senkron Kontrolü (codegen yerine guard) (30 Mayıs 2026)

**Karar:** Marka paletinin 3-dosya senkronu, codegen ile tek-literal'a indirgenmek yerine, sıfır-bağımlılık bir **doğrulayıcı** (`check-tokens-sync.mjs`, `npm run lint:tokens`, CI) ile korunur. `mfa-tokens.scss` kanonik kaynaktır; diğer ikisi ona uyar.

**Gerekçe:** PrimeNG preset ve `data:` SVG, tanım anında `var()` okuyamaz → marka hex'i 3 yerde literal olmak zorunda. Tam codegen kalıcı build karmaşası getirir ve nadir değişen (devlet kurumsal kimliği) bir palet için orantısız. Guard, drift'i build-zamanında yakalar; literal dosyalar insan-okur kalır.

**Etki:** Biri `mfa-tokens.scss`'i güncelleyip diğerini unutursa CI kırmızı döner. Palet bütünlüğü artık makine-garantili.

---

## Oturum 13 — 30 Mayıs 2026

### Test Altyapısı — Zoneless Birim Testleri + Governance Script Testleri (Tamamlandı)

**Bağlam:** Template `v1.0.0` production-ready idi ama otomatik test yoktu (`npm test` placeholder). Önceki oturumda plan onaylandı fakat GateGuard write'ları blokladı + maliyet patladı, hiçbir dosya yazılamadı. Bu oturumda GateGuard fact-force kapısına her write/edit öncesi gerekli facts sunularak ilerlendi; sıfır yeni paket hedefi korundu.

- [x] **`karma.conf.js`:** `ChromeHeadlessNoSandbox` launcher (`--no-sandbox --disable-gpu --disable-dev-shm-usage`) + disconnect/no-activity timeout toleransları (CI/yavaş makine kararlılığı). Proje zone.js KULLANMADIĞI için hiçbir yerde `zone.js/testing` import edilmez. `angular.json` test hedefine `karmaConfig` bağlandı; `styles: []` + `assets: []` yapıldı (webpack sass-loader `@/` alias'ını çözemiyor — birim testleri global stile ihtiyaç duymaz).
- [x] **`package.json`:** `test` → `ng test --no-watch --browsers=ChromeHeadlessNoSandbox`; `test:watch` → `ng test`; `test:scripts` → `node --test scripts/__tests__/*.test.mjs` (dizin formu Node 22.14'te "module not found" verdiği için Node-native glob formu kullanıldı).
- [x] **8 zoneless spec (45 test, hepsi yeşil):** `settings.service` (11) · `translate.service` (6, HttpTestingController + PrimeNG stub) · `translate.pipe` (4, fake servis) · `auth.guard` (2) · `auth.interceptor` (3, `withInterceptors`) · `permission.service` (7) · `auth.service` (8, güvenlik çekirdeği: depo/state/devLogin/handleCallback state doğrulaması — window.location atayan SSO yolları hariç) · `extractRoles` (4, JWT claim öncelik sırası). Ortak desen: `provideZonelessChangeDetection()` + effect'leri `ApplicationRef.tick()` ile flush; async load testlerinde `settle()` mikro-görev boşaltma.
- [x] **Governance script test edilebilirliği:** `check-palette.mjs` + `check-tokens-sync.mjs`'e `MFA_LINT_ROOT` env override'ı eklendi (ROOT'u sahte fixture köküne yönlendirir). `scripts/__tests__/*.test.mjs` (saf-Node `node:test`, 10 test): senkron palet → exit 0; drift (design-tokens/preset ayrışması) → exit 1; temiz kod → exit 0; HEX/TAILWIND/CDN/IMPORT ihlali → exit 1; `// mfa-ignore` bastırma → exit 0; sanctioned import → exit 0.
- [x] **CI (`ci.yml`):** job adı `lint + test + build`; build'den önce `test:scripts` ve `test` adımları eklendi. Sıra: `lint:palette` → `lint:tokens` → snippet tazeliği → `test:scripts` → `test` → `build`.
- [x] **Latent bug düzeltmesi (testin yakaladığı):** `SettingsService.applyDarkClass` — `document.startViewTransition()` hızlı tema değişiminde önceki geçişi iptal edince `ready`/`finished` promise'i `AbortError: Transition was skipped` ile reddediyor; yakalanmadığı için "unhandled promise rejection" seli oluşturup Karma'yı `afterAll`'da boğuyordu. `transition?.ready?.catch?.(()=>{})` + `finished` ile yutuldu — görsel davranış değişmedi, gerçek tarayıcıda da konsol gürültüsü engellendi.
- [x] **Doğrulama:** `npm run test:scripts` 10/10 ✓ · `npm test` 45/45 SUCCESS ✓ · `lint:palette` 0 ihlal (69 dosya) ✓ · `lint:tokens` senkron ✓ · `npm run build` BAŞARILI (18.0 sn, sadece mevcut Quill CommonJS uyarısı) ✓. Sıfır yeni paket.

#### K-025 — Test Stratejisi: Zoneless Karma + Saf-Node Governance Testleri (30 Mayıs 2026)

**Karar:** Test altyapısı sıfır yeni paketle kurulur — zaten kurulu Karma+Jasmine ile zoneless birim testleri (`core/` davranışı) + `node:test` ile governance script testleri. Zoneless uyumu için her spec `provideZonelessChangeDetection()` sağlar ve effect'leri `ApplicationRef.tick()` ile flush eder (zone.js yok → `fakeAsync`/zone-tick yok). Karma test hedefi global SCSS yüklemez. Governance script'leri `MFA_LINT_ROOT` env override'ı ile fixture köklerine yönlendirilerek izole edilir.

**Gerekçe:** CLAUDE.md §3 sıfır-paket felsefesi test araçları için de geçerli; `jest`/`vitest`/`playwright` eklemek yasak ve gereksiz. Karma+Jasmine devDependency olarak zaten mevcuttu. Asıl risk olan davranış regresyonları (auth/i18n/settings) ve governance script'lerinin sessizce bozulması artık makine-garantili. Test, kurulum sırasında gerçek bir latent bug'ı (startViewTransition unhandled rejection) ortaya çıkardı.

**Etki:** `npm test` + `npm run test:scripts` CI kapısına eklendi; bu kapılar yeşil olmadan merge edilmez. Modül takımları aynı zoneless desenle kendi servislerini test eder.

---

#### K-026 — Navigasyon Tek-Modül Modeli (module-nav vs template-nav) (2 Haziran 2026)

**Karar:** Sidebar navigasyonu ikiye bölündü ve "her fork = TEK modül" felsefesine göre yeniden tasarlandı.
- `core/config/module-nav.config.ts` → `MODULE_NAV` (modülün kendi menüsü; fork ekibi düzenler; üretimde görünür). `NavItem`/`NavGroup` tipleri buraya taşındı.
- `core/config/template-nav.config.ts` → `TEMPLATE_NAV` (geliştirici referansı: Bileşen Kütüphanesi / Kurumsal Kimlik+Ayarlar / Örnek Sayfalar; collapsible; **yalnızca SSO boşken** görünür, üretimde gizli; fork dokunmaz).
- `navigation.config.ts` → saf `buildNavGroups(isDevMode)` + geriye-uyumlu `NAV_GROUPS`/`ROUTE_LABEL_*` re-export. `app.menu` → `buildNavGroups(!appEnv.ssoUrl())`.
- **Dashboard kaldırıldı.** `app.routes.ts`'te `{ path: '', component: Dashboard }` → `{ path: '', redirectTo: 'vize', pathMatch: 'full' }`; `pages/dashboard/` silindi. "/" modülün ana sayfasına yönlenir; fork kendi route'una çevirir.
- "Modüller" üst sekmesi ve "Sayfalar" karışık grubu kaldırıldı. Dağınık sayfalar Kurumsal Kimlik + Örnek Sayfalar olarak ikiye ayrıldı.
- Ayarlar sayfası korundu: dev sidebar'da Kurumsal Kimlik altında; üretimde topbar 3'lü hızlı-ayar grubundan (topbar'da ayrı `pi-cog` butonu yok — CLAUDE.md §15 düzeltildi).
- i18n: `menu.dev`, `menu.dev.corporate`, `menu.dev.examples`, `menu.module.vize` eklendi; `menu.dashboard`/`menu.modules`/`menu.pages` + `dashboard.*` temizlendi (`menu.home` notfound için korundu).

**Test:** `buildNavGroups` saf birim testi (`navigation.config.spec.ts`) — dev modda template eklenir, üretimde eklenmez (47 test toplam).

**Doküman:** Tasarım `docs/superpowers/specs/2026-06-02-navigation-restructure-design.md`, plan `docs/superpowers/plans/2026-06-02-navigation-restructure.md`. Türev modüller için taşıma: `docs/navigasyon-migrasyon-rehberi.md`.

**Etki:** Üretimde modül uygulaması tek-modüllük temiz bir sidebar gösterir (yalnız modülün nav'ı). Geliştirme sırasında tüm referans collapsible olarak elde kalır. Fork ekibi yalnızca `module-nav.config.ts` + redirect hedefini düzenler.

---

## Sırada — Sonraki Oturum

**Tamamlanan:** Phase 7B + 7C + 8 + 9 (9A–9D) + 10 + 11 (Modül İskeleti/Docker/README) + **Kurumsal Kimlik Bütünlüğü (font tek-kaynak + SVG amblem + CI)** + **Kütüphane Zenginleştirme (Oturum 12: +6 kurumsal desen → 14 kalıp, 131 snippet)** + **Test Altyapısı (Oturum 13: zoneless birim testleri 45 test / 8 spec + governance script testleri 10 test + CI)**

**Açık iş / değerlendirme:**
- ✅ `docker build` + runtime testi **Oturum 12'de doğrulandı** (84.5 MB imaj, `/health` 200, config.js env). Açık iş kalmadı.
- Gerçek Bakanlık amblemi geldiğinde `core/util/mfa-logo.ts` SVG içeriği gerçek asset ile değiştirilecek (template tasarımı buna hazır — tek dosya).
- Gelecekteki **Angular 22** migration'ında Signal Forms açılır (CLAUDE.md §2).

**Git durumu:** Oturum 12 değişiklikleri commit + push edildi (kullanıcı onayıyla). `main` branch, `origin/main` ile senkron. Template production-ready baseline olarak kapatıldı.
