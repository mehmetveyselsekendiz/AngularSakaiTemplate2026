...
**Tamamlanan:** Phase 7B + 7C + 8 + 9.0 + **9A + 9B** (Bileşen Vitrini tam kapsam)

**AKTİF ODAK: Phase 9 (Yeniden) — kalan alt-fazlar 9C + 9D**
...
- **Görev planı:** [`superpowers/plans/2026-05-25-phase-9-component-library.md`](superpowers/plans/2026-05-25-phase-9-component-library.md)
- **9C** — Gap analizi (Explore/general-purpose subagent, salt okuma): `/uikit/*` `primeng/*` import seti vs PrimeNG 21 tam liste → eksik bileşenler (öncelik form/tablo/overlay/feedback). Kullanıcıya liste sun, onaylananları ilgili sayfaya `<app-showcase>` olarak ekle.
- **9D** — `src/app/pages/uikit/patternsdemo.ts` — 8 kompozit kurumsal kalıp (Page Header, Tablo+Toolbar, Form Kartı, Empty State, Stat Cards, Filtre Çubuğu, Onay Akışı, Detay Paneli) + route + menü + i18n etiketi.
- **Kapanış:** GateGuard hook'unu geri aç (`.claude/settings.local.json` → `ECC_DISABLED_HOOKS` kaldır); `sakai-mfa-uyarlama-plani.md` §4B Phase 9 → TAMAMLANDI.

**Yeni desen (CLAUDE.md §14):** Yeni uikit örneği = `<app-showcase title snippetId [code]>` + `<!-- snippet:ID -->` marker + `snippet(id)` + `npm run snippets`.

**Sonraki fazlar:**
- Phase 10 — Responsive Audit (5 breakpoint, touch ≥44px, WCAG 2.1 SC 1.4.10)
- Phase 11 — İlk modül iskeleti (`features/vize/`) + modül fork rehberi + OpenShift Docker + README

**Bilinen küçük borç:** `timelinedemo.ts` "Templating" snippet'inde `/demo/images/product/game-controller.jpg` kırık yerel görsel (CDN değil, governance ihlali değil) — `svgPlaceholder`'a çevrilebilir.

**Git durumu:** Phase 9A (`0efa807`) + 9B (`03fa01c`) commit edildi ve push edilecek. `main` branch.
